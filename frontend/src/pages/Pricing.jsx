...
function Pricing() {
  ...
  const handlePaying = async (amount) => {
    try {
      ...
      setPaying(true);
      // fetch api for paying
      const result = await axios.post(
        serverUrl + "/api/credit/order",
        { amount },
        { withCredentials: true },
      );
      // redirect to new URL fro payment session
      if (result.data.url) {
...
function PricingCard({
  ...
  return (
    <motion.div
      onClick={() => setSelectedPrice(amount)} // set selected to amount
      whileHover={{ y: -4 }}
    ...
    >
      ...
      {/* Button */}
      <button
        disabled={isPayingThisCard} //if is paying disable it
        // stop bydefault behaviour if clicked
        onClick={(e) => {
          e.stopPropagation();
          onBuy(amount);
        }}

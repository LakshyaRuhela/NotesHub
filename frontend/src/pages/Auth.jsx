...
function Auth() {
  ...
  const handleGoogleAuth = async () => {
    try {
      const response = await signInWithPopup(auth, provider); // auth comes from firebase
      // console.log(response);
      // take data from firebase response and send it to server
      const User = response.user; // firebase response
      const name = User.displayName;
      const email = User.email; // get email
      // axois req for data send to server

...
function History() {
  const [topics, setTopics] = useState([]); // state for notes topic array
  const navigate = useNavigate();
  const { userData } = useSelector((state) => state.user);
  const credits = userData.credits; // to find credits
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // state for side bar
  const [activeNoteId, setActiveNoteId] = useState(null); // state for active note
  const [selectedNote, setSelectedNote] = useState(null); // for selected note
  ...
  useEffect(() => {
    ...
    myNotes(); // call function automatically
  }, []);
  ...
  const openNotes = async (noteId) => {
    ...
    try {
      ...
      // Fix: Always pass an object to FinalResult
      setSelectedNote(
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 px-6 py-8">
      ...
      >
        ...
        <div className="flex items-center gap-4 ">
          {/* If small screen show hamburger sign */}
          {!isSidebarOpen && (
            <button
          ...
          {isSidebarOpen && (
            ...
            >
              ...
              <div className="mb-4 space-y-1">
                ...
                {/* if no notes present show No Notes  */}
                {topics.length === 0 && (
